...
instance Pretty Term where
 ...
 pretty (Comb "." [a,b]) = "[" ++ prettyList a b ++ "]"  -- für den Spezialfall "." wird die Hilfsfunktion prettyList aufgerufen
  where
   prettyList :: Term -> Term -> String  -- pretty für den Rest einer Comb, also die Liste
 ...
 pretty (Comb f x) = f ++ "(" ++ (intercalate ", " (map pretty x)) ++ ")" -- Name von f wird behalten, map über Liste mit anschließender Separierung mit Kommas
